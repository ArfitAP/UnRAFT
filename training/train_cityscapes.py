...
from core.datasets import Cityscapes
...
def fetch_dataloader(args, TRAIN_DS='C+T+K+S+H'):
    ...
    aug_params = {'crop_size': [304, 608], 'min_scale': -0.2, 'max_scale': 0.4, 'do_flip': False}
    #train_dataset = Cityscapes(aug_params)
    
    # Colab:
    train_dataset = Cityscapes(aug_params, root='/content/Cityscapes')
...
def train(args):
    ...
    while should_keep_training:
        ...
        for _, (image1, image2) in enumerate(train_loader):
            optimizer.zero_grad()
            ...
            if total_steps % CHECK_FREQ == CHECK_FREQ - 1:
                PATH = '/content/drive/MyDrive/FAKS/10. sem/RAFT-selfsupervised/checkpoints/%d_%s.pth' % (total_steps + 1, args.name)
    ...
    PATH = '/content/drive/MyDrive/FAKS/10. sem/RAFT-selfsupervised/checkpoints/%s.pth' % args.name
    torch.save(model.state_dict(), PATH)
